...
// EMR bridge (Swing↔JavaFX)
import javafx.application.Platform;

/**
 * Swing GUI application for KCD Database Management
 * Provides a desktop interface for CRUD operations on KCD codes
 * 
 * Features:
 * - Full CRUD operations (Create, Read, Update, Delete)
 * - Search and filtering capabilities
 * - Copy to clipboard functionality
 * - EMR integration for saving records
...
public class KCDDatabaseManagerSwing extends JFrame {
    ...
    private static final int EMR_TARGET_AREA = 7; // Assessment area in EMR layout
    
    // UI Color scheme
    private static final Color PRIMARY_COLOR = new Color(0x4FC3F7); // Sky Blue 300
    private static final Color PRIMARY_HOVER = new Color(0x29B6F6); // Sky Blue 400
    private static final Color SUCCESS_COLOR = new Color(0x4CAF50); // Green
    private static final Color INFO_COLOR = new Color(0x2196F3); // Blue
    private static final Color WARNING_COLOR = new Color(0xFF9800); // Orange
    private static final Color DANGER_COLOR = new Color(0xF44336); // Red
    private static final Color SECONDARY_COLOR = new Color(0x37474F); // Blue Grey
    ...
    private JButton saveToEmrButton;
    ...
    private void initializeTable() {
        ...
        // Setup table sorter for filtering
    ...
    private void initializeStatusBar() {
        ...
        statusLabel.setForeground(new Color(0x333333)); // Dark gray for better readability
    }
    ...
    private void setupTableEventHandlers() {
        ...
        // Double-click handler for viewing details
    ...
    private void saveSelectedToEMR() {
        try {
            ...
            // Create EMR-formatted entry with timestamp
            String timestamp = LocalDate.now().format(ISO_DATE_FORMAT);
            String emrEntry = String.format("\n< KCD > %s\n%s", timestamp, record.toEMRFormat());
            
            // Validate EMR bridge connection
            // Note: This requires the actual EMR bridge implementation
            // For now, we'll show what would be saved
            showEMRPreview(emrEntry, record);
    ...
    private void showEMRPreview(String emrEntry, KCDRecord record) {
        ...
        if (result == JOptionPane.OK_OPTION) {
            // Here you would actually save to EMR
    ...
    private KCDRecord getSelectedRecord() {
        ...
        return new KCDRecord(
            getTableValue(modelRow, 0), // classification
            getTableValue(modelRow, 1), // disease_code
            getTableValue(modelRow, 2), // check_field
            getTableValue(modelRow, 3), // korean_name
            getTableValue(modelRow, 4), // english_name
            getTableValue(modelRow, 5)  // note
        );
    }
    ...
    /**
     * Data model class for KCD records
     */
    ...
    private static class DatabaseManager {
        ...
        public static boolean addRecord(KCDRecord record) throws SQLException {
            String sql = "INSERT INTO kcd_codes(classification, disease_code, check_field, korean_name, english_name, note) " +
                        "VALUES(?, ?, ?, ?, ?, ?)";
        ...
        public static boolean updateRecord(String originalDiseaseCode, KCDRecord record) throws SQLException {
            String sql = "UPDATE kcd_codes SET classification=?, disease_code=?, check_field=?, " +
                        "korean_name=?, english_name=?, note=? WHERE disease_code=?";
    ...
    /**
     * Dialog for adding/editing KCD records
     */
    private static class KCDRecordDialog extends JDialog {
        ...
        public KCDRecordDialog(JFrame parent, String title, KCDRecord initialData) {
            ...
            fields = new JTextField[labels.length - 1]; // Note field handled separately
        ...
        private void createDialogLayout(String[] labels, KCDRecord initialData) {
            ...
            for (int i = 0; i < labels.length; i++) {
                ...
                if (i == labels.length - 1) { // Note field
        ...
        private boolean validateInput() {
            ...
            if (fields[1].getText().trim().isEmpty()) { // Disease Code is required
            ...
            if (fields[3].getText().trim().isEmpty()) { // Korean Name is required
                JOptionPane.showMessageDialog(this, 
                    "Korean Name is required.", 
                    "Validation Error", 
                    JOptionPane.ERROR_MESSAGE);
        ...
        public KCDRecord getRecord() {
            return new KCDRecord(
                fields[0].getText().trim(), // classification
                fields[1].getText().trim(), // disease_code
                fields[2].getText().trim(), // check_field
                fields[3].getText().trim(), // korean_name
                fields[4].getText().trim(), // english_name
                noteTextArea.getText().trim() // note
            );
        }
    }
